...
public class ShiroRealm extends AuthorizingRealm implements Serializable {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken)
		...
		sql.append("select u.*,o.org_name orgname,d.name deptname,p.name personname from ");
		sql.append("(select t_u.* from yy_user t_u where t_u.status='1' and t_u.loginname='")
				.append(token.getUsername()).append("' ) u ");
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		try {
			 ...
			 * 所以，要为了达到权限控制的目的，在缓存人员权限的时候，需要在每个权限后面加上:view。表示只有查看权限。 其他操作权限的控制要根据需要进行配置和添加
			 */
			// 菜单权限
