...
public class UserController extends BaseController<UserEntity> {
	...
	/**
	 * 获取用户的选中组织单元的树数据 @Title: getUserOrg @author liusheng @date 2016年2月18日 下午7:19:25 @param @param
	 * userId @param @return @param @throws Exception 设定文件 @return List<TreeBaseBean> 返回类型 @throws
	 */
	@ResponseBody
	@RequestMapping(value = "/getUserOrg")
	public List<TreeBaseBean> getUserOrg(String userId) throws Exception {
		return getService().getUserOrg(userId);
	}

	/**
	 * 保存用户关联的业务单元 @Title: saveUserOrg @author liusheng @date 2016年2月18日 下午8:05:31 @param @param userId @param @param
	 * checkedIds @param @return 设定文件 @return JsonBaseBean 返回类型 @throws
	 */
	@ResponseBody
	@RequestMapping(value = "/saveUserOrg")
	public JsonBaseBean saveUserOrg(String userId, String checkedIds) {
		return getService().saveUserOrg(userId, checkedIds);
	}
	...
	@ResponseBody
	private ActionResultModel changePwd(@RequestParam String oldpassword, @RequestParam String newpassword) {
