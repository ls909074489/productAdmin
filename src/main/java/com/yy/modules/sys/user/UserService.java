...
import com.yy.frame.security.SecurityConfig;
import com.yy.frame.security.ShiroUser;
import com.yy.frame.service.BaseServiceImpl;
import com.yy.frame.utils.Digests;
import com.yy.frame.utils.Encodes;
import com.yy.frame.utils.RequestUtil;
import com.yy.modules.sys.func.FuncEntity;
import com.yy.modules.sys.func.action.FuncActionEntity;
import com.yy.modules.sys.org.OrgEntity;
import com.yy.modules.sys.org.OrgRef;
...
import com.yy.modules.sys.userrole.UserRoleService;
...
public class UserService extends BaseServiceImpl<UserEntity, String> {
	...
	public List<TreeBaseBean> getUserOrg(String userId) throws Exception {
		List<OrgEntity> allList = (List<OrgEntity>) orgService.findAll(new Sort(Sort.Direction.ASC, "org_code"));
		StringBuilder sql = new StringBuilder("");
		sql.append("select ur.* from yy_user_org ur ");
		sql.append("where ur.status=1 and ur.user_id='").append(userId).append("'");
		List<UserOrgEntity> rfList = dbDao.find(UserOrgEntity.class, sql.toString());
	...
	public JsonBaseBean saveUserOrg(String userId, String checkedIds) {
		JsonBaseBean json = new JsonBaseBean();
		// StringBuilder sql=new StringBuilder("delete from yy_user_org where ");
		// sql.append(" user_id='").append(userId).append("'");
		// dbDao.update(sql.toString());
		try {
			userOrgService.deleteByUserid(userId);
			if (!StringUtils.isEmpty(checkedIds)) {
				UserOrgEntity userOrg = null;
				String[] arr = checkedIds.split(",");
				for (String orgId : arr) {
					userOrg = new UserOrgEntity();
					userOrg.setUser_id(userId);
					userOrg.setPk_corp(orgId);
	...
	public List<UserEntity> getUserBySql() {
		...
		try {
			...
			// org=new OrgRef();
			// org.setUuid(user.getOrgid());
			// org.setOrg_name(user.getOrgname());
			// user.setCorp(org);
			// }
			// if(!StringUtils.isEmpty(user.getDeptid())){
			// department=new DepartmentEntity();
			// department.setUuid(user.getDeptid());
			// department.setName(user.getDeptname());
			// user.setDepartment(department);
			// }
			// if(!StringUtils.isEmpty(user.getPersonid())){
			// person=new PersonEntity();
			// person.setUuid(user.getPersonid());
			// person.setName(user.getPersonname());
	...
	public ActionResultModel<UserEntity> addUser(UserEntity entity) {
		...
		try {
			...
			entity = doAdd(entity);
			// if (!StringUtils.isEmpty(entity.getPersonid())) {// 修改人员关联用户的关系
			// PersonEntity person = personService.getOne(entity.getPersonid());
			// person.setRelation_user(entity);
	...
	/**
	 * 修改用户 @Title: updateUser @author liusheng @date 2016年6月12日 下午3:09:07 @param @param entity @param @return
	 * 设定文件 @return ActionResultModel<UserEntity> 返回类型 @throws
	 */
	// public ActionResultModel<UserEntity> updateUser(UserEntity entity) {
	// ActionResultModel<UserEntity> arm = new ActionResultModel<UserEntity>();
	// try {
	// UserEntity oldUser=getOne(entity.getUuid());
	// if(!StringUtils.isEmpty(oldUser.getPersonid())){
	// PersonEntity person=personService.getOne(oldUser.getPersonid());
	// person.setRelation_user(null);//将原来关联的人员的登录用户清空
	// personService.save(person);
	// }
	//
	//
	...
	public List<UserEntity> findSelSendUser(String groupId) {
		List<UserEntity> list = new ArrayList<>();
		StringBuilder sql = new StringBuilder();
		sql.append("select u.*,d.name deptname  from cs_send_user_relation ur ");
		sql.append("left join yy_user u on u.uuid=ur.user_id ");
	...
	/**
	 * 获取可选的派单人的用户 @Title: findAllSendUser @author liusheng @date 2016年6月28日 下午9:25:14 @param @param
	 * groupId @param @return 设定文件 @return List<UserEntity> 返回类型 @throws
	 */
	public List<UserEntity> findAllSendUser(String groupId) {
		List<UserEntity> list = new ArrayList<>();
		StringBuilder sql = new StringBuilder();
		sql.append("select u.*,d.name deptname from (select * from yy_user t_u where not exists(");
		sql.append("select * from cs_send_user_relation ur where ur.senduser_id='").append(groupId).append("'");
		sql.append(" and ur.user_id=t_u.uuid) and t_u.status=1 and t_u.orgid='")
				.append(ParameterUtil.getParamValue("zhmzkj", Constants.DEFAULT_ORG_ID)).append("') u ");
		// sql.append("left join yy_org o on o.uuid=u.orgid "); ,o.org_name orgname
		sql.append("left join yy_department d on d.uuid=u.deptid ");
		sql.append("order by d.code");
		try {
