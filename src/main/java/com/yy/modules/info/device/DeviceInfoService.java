...
//		}
	...
	public DeviceInfoEntity updateDevice(DeviceInfoEntity bean, List<MultipartFile> files) {
		...
		if(!(bean.getInterval()!=null&&!StringUtils.isEmpty(bean.getInterval().getUuid()))){
			entity.setInterval(null);
		}else{
			entity.setInterval(bean.getInterval());
		}
		if(!(bean.getSupplier()!=null&&!StringUtils.isEmpty(bean.getSupplier().getUuid()))){
			entity.setSupplier(null);
		}else{
			entity.setSupplier(bean.getSupplier());
		}
