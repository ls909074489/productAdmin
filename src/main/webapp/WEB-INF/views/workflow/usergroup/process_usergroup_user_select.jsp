	...
	function onRefreshSelect() {
	 ...
	 //显示自定义的行按钮
	  ...
	  //显示自定义的行按钮
		...
		function loadList(){
			$.ajax({
				url : '${serviceurl}/findAllProcessUserGroupId?groupId=${groupId}',
		...
		function loadListSelect(){
			$.ajax({
				url : '${serviceurl}/findByProcessUserGroupId?groupId=${groupId}',
		...
		function selObj(aData, iDataIndex, nRow){
			...
			var groupId='${groupId}';
			$.ajax({
				 url: '${ctx}/process/usergrouprelation/selecUser',
			       type: 'post',
			       data : {
						'groupId' : groupId,
						'userId':userId
