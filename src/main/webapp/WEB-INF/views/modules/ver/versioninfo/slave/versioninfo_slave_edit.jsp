...
<body>
	<div id="yy-page-edit" class="container-fluid page-container page-content" >
		<div class="row yy-toolbar">
			...
			<c:if test="${empty entity.billstatus or entity.billstatus eq 1 or entity.billstatus eq 4}">
	...
	<div>
		<form id="yy-form-edit" class="form-horizontal yy-form-edit">
			...
			<div class="row">
				...
				<div class="col-md-4">
					<div class="form-group">
						<label class="control-label col-md-4 required">导入</label>
						<div class="col-md-8">
							<div class="input-group input-icon right">
	...
	var jsonResp = jQuery.parseJSON('${colsJson}');
		var nRowEle=null;//编辑行
		var aDataEle=null;//编辑行
		var iDataIndexEle=null;//编辑行
		/* 子表操作 */
		var _subTableYaoxinCols = [{
			...
			render : function(data, type, full) {
				...
				+ "<button id='yy-btn-editsub-row' class='btn btn-xs btn-info edit' data-rel='tooltip' title='编辑' "+passDiable+"><i class='fa fa-edit'></i></button>"
				+ "<button id='yy-btn-delsub-row' class='btn btn-xs btn-danger' subType='tx' data-rel='tooltip' title='删除'><i class='fa fa-trash-o'></i></button>"
				+ "<button id='yy-btn-pass-row' class='btn btn-xs btn-info edit' data-rel='tooltip' title='通过' "+passDiable+"><i class='fa fa-check'></i></button>"
				+ "<button id='yy-btn-question-row' class='btn btn-xs btn-danger delete' data-rel='tooltip' title='质疑' "+quesDiable+"><i class='fa fa-hourglass-2'></i></button>"
				+ "</div>";
	        }
		}, {
			data : 'substatus',
					...
					if(cosList[i].isDisplay=='1'){
						if(cosList[i].columnName=='pointno'){
							_subTableYaoceCols.push({data : cosList[i].columnName,width : cosList[i].width,orderable : false,
								render : function(data, type, full) {
									if(full.haslinks!=null&&full.haslinks=='1'){
										return '<a onclick="showLinksInfo(\''+full.uuid+'\');">'+data+'</a>';
									}else{
										return '<a onclick="showLinksInfo(\''+full.uuid+'\');">'+data+'&nbsp;<i class="fa fa-search-plus"></i></a>';
									}
								}});
		...
		var _pageNumberYx,_pageNumberYc,_pageNumberYk,_pageNumberYt;
		...
		function selRowAction(nRow, aData, iDataIndex) {
			...
			$('#yy-btn-editsub-row', nRow).click(function() {
				nRowEle=nRow;//编辑行
				aDataEle=aData;//编辑行
				iDataIndexEle=iDataIndex;//编辑行
				layer.open({
					title:"信息",
				    type: 2,
				    area: ['400px', '400px'],
				    shadeClose : false,
					shade : 0.8,
				    content: '${slaveserviceurl}/toEditSubPage?subId='+aData.uuid
				});
			});
			
			
			$('#yy-btn-delsub-row', nRow).click(function() {
					layer.confirm("是否确实要删除？", function(index) {
						layer.close(index);
						var delExInfoLoading = layer.load(2);
						$.ajax({
							type : "POST",
							data :{"pks": aData.uuid},
							url : "${serviceurlsub}/delete",
							async : true,
							dataType : "json",
							success : function(data) {
								layer.close(delExInfoLoading);
								if (data.success) {
									YYUI.succMsg(YYMsg.alertMsg('sys-success-todo',['删除']));
									if(aData.type=='tx'){
										loadSubYaoxin('${entity.mainindex}');
									}else if(aData.type=='tc'){
										loadSubYaoce('${entity.mainindex}');
									...
									YYUI.promAlert(YYMsg.alertMsg('sys-fail-todo',['删除'])+data.msg);
								}
							},
							error : function(data) {
								layer.close(delExInfoLoading);
		...
		//回调修改行数据
		function callBackUpdateRowData(subType,data){
			var jsonData = jQuery.parseJSON(data.content);//数据库保存的信息
			if(jsonResp){
				var cosList=null;
				if(subType=='yx'){
					cosList=jsonResp.txList;
				}else if(subType=='yc'){
					cosList=jsonResp.tcList;
				}else if(subType=='yk'){
					cosList=jsonResp.tkList;
				}else if(subType=='yt'){
					cosList=jsonResp.ttList;
				}else if(subType=='tx'){
					cosList=jsonResp.txList;
				}else if(subType=='tc'){
					cosList=jsonResp.tcList;
				...
				if(cosList!=null&&cosList.length>0){
					...
					obj.type=data.type;
					obj.compareResult=data.compareResult;
					obj.haslinks=data.haslinks;
					for (j = 0; j < cosList.length; j++) {
						if(cosList[j].isDisplay=='1'){
							var jsonColName=cosList[j].columnName;
							if(typeof (jsonData[jsonColName]) != "undefined"){
			...
			console.info(obj);
			if(subType=='yx'){
				_subYaoxinTableList.row(nRowEle).data(obj);
			}else if(subType=='yc'){
				_subYaoceTableList.row(nRowEle).data(obj);
			}else if(subType=='yk'){
				_subYaokongTableList.row(nRowEle).data(obj);
			}else if(subType=='yt'){
				_subYaotiaoTableList.row(nRowEle).data(obj);
			}else if(subType=='tx'){
				_subYaoxinTableList.row(nRowEle).data(obj);
			}else if(subType=='tc'){
				_subYaoceTableList.row(nRowEle).data(obj);
			...
			selRowAction(nRowEle, obj, iDataIndexEle);//重新绑定方法
			var jqTds = $('>td', nRowEle);
			jqTds[0].innerHTML = (iDataIndexEle+1);//重新设置序号
		}
		...
		function bindButtonAction(){
			...
			$('#pkgPath-select-btn').on('click', function() {
				layer.open({
					...
					shade : 0.8,
					area : [ '90%', '90%' ],
					content : '${serviceurl}/toUploadFileNew?callBackMethod=window.parent.callBackUploadFile&mainindex='+$("#mainindex").val()
				});
			});
			
			$('#pkgPath-file-btn').on('click', function() {
				layer.open({
					type : 2,
					title : '请选择文件',
					shadeClose : false,
		...
		function validateForms(){
			validata = $('#yy-form-edit').validate({
				...
				rules : {
					...
					'subver' : {required : true,maxlength : 200},
		...
		function callBackUploadFile(data){
			...
			if(data.records!=null&&data.records.length>0){
				...
				_subYaokongTableList.draw(); //服务器分页
				_subYaotiaoTableList.draw(); //服务器分页
				//_subTeshuyxTableList.draw(); //服务器分页
			}else{
