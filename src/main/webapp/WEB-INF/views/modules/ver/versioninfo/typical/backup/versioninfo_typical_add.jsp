...
<body>
	...
	var jsonResp = jQuery.parseJSON('${colsJson}');
		...
		function bindButtonAction(){
			...
			//添加子表
			$("#addNewSubYaoxin").on('click',  function() {addNewSubFunc('yx');});//添加子表
			$("#addNewSubYaoce").on('click',  function() {addNewSubFunc('yc');});//添加子表
			$("#addNewSubYaokong").on('click',  function() {addNewSubFunc('yk');});//添加子表
			$("#addNewSubYaotiao").on('click',  function() {addNewSubFunc('yt');});//添加子表
			$("#addNewSubTeshuYx").on('click',  function() {addNewSubFunc('tx');});//添加子表
		}
		...
		function callBackUploadFile(data){
			...
			if(data.records!=null&&data.records.length>0){
				...
				loadSubTeshuyx(data.records[0].mainindex);
