	...
	<script type="text/javascript">
		...
		var _tableCols = [ {
							...
							render : function(data, type, full) {
								...
								//+ "<button id='yy-btn-download-row' class='btn btn-xs green btn-sm'><i class='fa fa-chevron-up'></i> 导出CSV</button>"
								+ "</div>";
							}
						}/* , {
							data : 'billstatus',
		...
		function onAdd() {
			...
			layer.open({
				...
				closeBtn : 0,//关闭按钮
				area : [ '100%', '100%' ],
				content : '${serviceurl}/onAddTypical?'+ _addParam, //iframe的url
			});
			onAddAfter();
		}
		...
		function onEditRow(aData, iDataIndex, nRow) {
			...
			layer.open({
				...
				closeBtn : 0,//关闭按钮
				area : [ '100%', '100%' ],
				content : '${serviceurl}/onEditTypical?uuid=' + aData.uuid + '&' + _editParam, //iframe的url
			});
		}
		...
		function onViewDetailRow(data, rowidx, row) {
			layer.open({
				...
				closeBtn : 0,//关闭按钮
				area : [ '100%', '100%' ],
				content : '${serviceurl}/onDetailTypical?uuid=' + data.uuid + '&' + _detailParam, //iframe的url
			});
		}
		//下载csv
		function onDownloadRow(data, rowidx, row){
			var downLoadLoading = layer.load(2);
			$.ajax({
				type : "POST",
				data :{"mainId": data.uuid},
