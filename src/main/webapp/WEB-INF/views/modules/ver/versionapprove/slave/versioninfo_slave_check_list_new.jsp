...
<body>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			bindListActions();
			serverPage("${serviceurl}/dataSlaveCheck?approveType=${approveType}");
		...
		function onEditRow(aData, iDataIndex, nRow) {
			...
			if (aData.billstatus!=null&&aData.billstatus ==5) {
				YYUI.promMsg("已通过的数据不能修改");//已经提交或者审核的数据不能修改。
				return;
			}else{
				layer.open({
					type : 2,
					...
					closeBtn : 0,//关闭按钮
					area : [ '100%', '100%' ],
					content : '${serviceurl}/onEdit?uuid=' + aData.uuid + '&' + _editParam+"&approveType=${approveType}", //iframe的url
				});
			}
		}
		...
		function onViewDetailRow(data, rowidx, row) {
			layer.open({
				...
				closeBtn : 0,//关闭按钮
				area : [ '100%', '100%' ],
				content : '${serviceurl}/onDetail?uuid=' + data.uuid + '&' + _detailParam+"&approveType=${approveType}", //iframe的url
			});
		}
