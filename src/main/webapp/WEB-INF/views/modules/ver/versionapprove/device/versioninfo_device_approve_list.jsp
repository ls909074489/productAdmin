...
<body>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			bindListActions();
			serverPage("${serviceurl}/dataDeviceApprove");
		});
		...
		function onEditRow(aData, iDataIndex, nRow) {
			...
			if (aData.billstatus !=null && aData.billstatus == '3') {
				layer.open({
					...
					closeBtn : 0,//关闭按钮
					area : [ '100%', '100%' ],
					content : '${serviceurl}/onEditApproveDevice?uuid=' + aData.uuid + '&' + _editParam, //iframe的url
				});
			}else{
		...
		function onViewDetailRow(data, rowidx, row) {
			layer.open({
				...
				closeBtn : 0,//关闭按钮
				area : [ '100%', '100%' ],
				content : '${serviceurl}/onDetailApproveDevice?uuid=' + data.uuid + '&' + _detailParam, //iframe的url
			});
		}
